...
class _NoteDetailPageState extends State<NoteDetailPage> {
  ...
  Future<void> _createRelation(int targetNoteId, String relationType) async {
    ...
    if (!mounted) return;
    if (token == null || token.isEmpty) {
      messenger.showSnackBar(const SnackBar(content: Text('登录状态已失效，请重新登录')));
      return;
    }
    ...
    try {
      ...
      if (r.statusCode == 201) {
        messenger.showSnackBar(const SnackBar(content: Text('关系创建成功')));
      ...
      if (!mounted) return;
      messenger.showSnackBar(SnackBar(content: Text('网络请求失败：$e')));
    }
  }
  ...
  Future<void> _deleteRelation(int relationId) async {
    ...
    try {
      ...
      if (r.statusCode == 200) {
        messenger.showSnackBar(const SnackBar(content: Text('关系删除成功')));
        await _fetchRelatedNotes();
      } else {
        messenger.showSnackBar(
  ...
  Future<void> _showNoteSelector(String relationType) async {
    ...
    try {
      ...
      if (!mounted) return;
      messenger.showSnackBar(SnackBar(content: Text('加载笔记列表失败：$e')));
      return;
    }

    if (allNotes.isEmpty) {
      messenger.showSnackBar(const SnackBar(content: Text('没有其他笔记可以关联')));
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final tags =
        (note['tags'] as List<dynamic>? ?? []).map((e) => e.toString()).toList();
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Text('相关笔记', style: Theme.of(context).textTheme.titleMedium),
                const Spacer(),
                IconButton(
