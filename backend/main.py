...
def _ensure_legacy_schema() -> None:
    ...
    try:
        with engine.begin() as conn:
            ...
            # CAL-02: smart_folders table
            _sf = conn.exec_driver_sql(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='smartfolder'"
            ).fetchall()
            if not _sf:
                pass  # Table created by create_all()
...
class TagManageItem(SQLModel):
    """Schema for tag management page - includes all note IDs using this tag (TAG-04)."""
    id: int          # minimal notetag.id for this user's this tag name
    name: str        # tag display name
    count: int = 0   # usage count (number of notes)
    note_ids: List[int] = Field(default_factory=list)  # IDs of notes using this tag


class TagRename(SQLModel):
    """Request body for renaming a tag (TAG-04)."""
    new_name: str
...
# CAL-02: Smart Folders (saved filter presets)
class SmartFolder(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    user_id: int = Field(foreign_key="user.id", index=True)
    name: str
    # Stored filter params as JSON string, mirrors /notes query params
    filter_params: str  # JSON: {"q":"...","tag":"...","tag_ids":"...","date_from":"...","date_to":"...","filter":"...","sort":"...","order":"...","match":"..."}
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)


class SmartFolderCreate(SQLModel):
    name: str
    filter_params: str  # JSON string of filter params


class SmartFolderUpdate(SQLModel):
    name: Optional[str] = None
    filter_params: Optional[str] = None  # JSON string


class SmartFolderRead(SQLModel):
    id: int
    user_id: int
    name: str
    filter_params: str
...
# Fix ForwardRef for NoteRead (tags/attachments reference models defined after NoteRead)
NoteRead.update_forward_refs()
...
async def chat_completions_proxy(request: Request):
    """透传代理：uni-app 前端 → 本后端 → DashScope compatible API。
    解决两个问题：
    1. 浏览器跨域（CORS）限制，前端无法直接调 DashScope
    2. API Key 不暴露到前端
    """
    dash_key = os.getenv("DASHSCOPE_API_KEY") or os.getenv("DASHSCOPE")
    if not dash_key:
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail="DashScope 请求超时，请稍后重试")
    except requests.RequestException as e:
        raise HTTPException(status_code=502, detail=f"DashScope 请求失败: {str(e)}")


@app.post("/v1/agent/chat")
async def agent_chat(request: Request):
    """
    Mentor agent endpoint with tool-calling skills.

    Replaces the simple proxy: each mentor is now an Agent that can
    call search_memory (semantic retrieval over user fragments) when needed.

    Request body:
        messages      — conversation history (without system)
        mentor_prompt — the perspective/viewpoint prompt (task-focused)
        fragments     — list of user personal fragment objects
    """
    from backend.agent import run_agent  # lazy import to keep startup fast
    ...
    mentor_prompt: str = body.get("mentor_prompt", "")
    fragments: list = body.get("fragments", [])
    ...
    try:
        ...
        raise HTTPException(status_code=504, detail="DashScope 请求超时")
    except Exception as e:
...
@app.get("/tags", response_model=List[TagManageItem])
def list_tags(
    session: Session = Depends(get_session),
    current_user: User = Depends(get_current_user),
):
    """Return all unique tags for the current user with counts and note IDs (TAG-04)."""
    rows = session.exec(
        select(NoteTag.tag, func.count(NoteTag.note_id).label("cnt"))
        .where(NoteTag.user_id == current_user.id)
        .group_by(NoteTag.tag)
        .order_by(func.count(NoteTag.note_id).desc())
    ).all()

    result: List[TagManageItem] = []
    for r in rows:
        min_id_row = session.exec(
            select(func.min(NoteTag.id)).where(
                (NoteTag.user_id == current_user.id) & (NoteTag.tag == r.tag)
        ...
        # Get all note IDs for this tag
        note_ids = [
            nt.note_id for nt in session.exec(
                select(NoteTag.note_id).where(
                    (NoteTag.user_id == current_user.id) & (NoteTag.tag == r.tag)
                ).distinct()
            ).all()
        ]
        result.append(TagManageItem(
            id=min_id_row or 0,
            name=r.tag,
            count=r.cnt,
            note_ids=note_ids,
...
@app.patch("/tags/{tag_id}", response_model=TagManageItem)
def rename_tag(
    tag_id: int,
    body: TagRename,
    session: Session = Depends(get_session),
    ...
    """Rename a tag across all notes and update note content (TAG-04)."""
    # Find the tag record - must belong to current user
    tag_record = session.exec(
        select(NoteTag).where(
            (NoteTag.id == tag_id) & (NoteTag.user_id == current_user.id)
        )
    ).first_or_none()
    if not tag_record:
        raise HTTPException(status_code=404, detail="tag not found")

    old_name = tag_record.tag
    ...
    if old_name == new_name:
        ...
        count = session.exec(
            select(func.count(NoteTag.note_id)).where(
                (NoteTag.user_id == current_user.id) & (NoteTag.tag == old_name)
            )
        ).one()
        return TagManageItem(id=tag_id, name=old_name, count=count, note_ids=[])

    # Get all affected note IDs
    affected_note_ids = [
        r.note_id for r in session.exec(
            select(NoteTag.note_id).where(
                (NoteTag.user_id == current_user.id) & (NoteTag.tag == old_name)
            )
        ).all()
    ...
    # Update all NoteTag rows for this user+old_name -> new_name
    for note_id in affected_note_ids:
        existing_new = session.exec(
            select(NoteTag).where(
                (NoteTag.note_id == note_id) & (NoteTag.user_id == current_user.id) & (NoteTag.tag == new_name)
            )
        ).first_or_none()
        if existing_new:
            # Merge: delete the old one, keep the new (dedup)
            to_delete = session.exec(
                select(NoteTag).where(
                    (NoteTag.note_id == note_id) & (NoteTag.user_id == current_user.id) & (NoteTag.tag == old_name)
                )
            ).all()
            for t in to_delete:
                session.delete(t)
    ...
    # Update Note.content: replace #old_name with #new_name
    for note_id in affected_note_ids:
        note = session.get(Note, note_id)
        if note and note.content:
            # Replace #old_name with #new_name (whole-word style)
            pattern = re.compile(r'(?<![^\s])#' + re.escape(old_name) + r'\b')
            new_content = pattern.sub('#' + new_name, note.content)
    ...
    count = session.exec(
        select(func.count(NoteTag.note_id)).where(
            (NoteTag.user_id == current_user.id) & (NoteTag.tag == new_name)
        )
    ).one()
    note_ids = [
        r.note_id for r in session.exec(
    ...
    return TagManageItem(id=tag_id, name=new_name, count=count, note_ids=note_ids)


@app.post("/tags/merge", response_model=TagManageItem)
def merge_tags(
    source_tag: str,
    target_tag: str,
    session: Session = Depends(get_session),
    ...
    """Merge source_tag into target_tag: delete source tag records, update content (TAG-04)."""
    source_tag = source_tag.strip()
    target_tag = target_tag.strip()
    if not source_tag or not target_tag:
        raise HTTPException(status_code=400, detail="source and target tag names required")
    if source_tag == target_tag:
        raise HTTPException(status_code=400, detail="source and target must be different")

    # Get affected note IDs for source tag
    affected_note_ids = [
        r.note_id for r in session.exec(
            select(NoteTag.note_id).where(
                (NoteTag.user_id == current_user.id) & (NoteTag.tag == source_tag)
            )
        ).all()
    ]

    # For each affected note: if target tag already exists, delete source; else rename source to target
    for note_id in affected_note_ids:
        existing_target = session.exec(
            select(NoteTag).where(
                (NoteTag.note_id == note_id) & (NoteTag.user_id == current_user.id) & (NoteTag.tag == target_tag)
            )
        ).first_or_none()
        if existing_target:
            # Remove source tag entries for this note
            to_delete = session.exec(
                select(NoteTag).where(
                    (NoteTag.note_id == note_id) & (NoteTag.user_id == current_user.id) & (NoteTag.tag == source_tag)
                )
            ).all()
            for t in to_delete:
                session.delete(t)
    ...
    # Update Note.content: replace #source_tag with #target_tag
    pattern = re.compile(r'(?<![^\s])#' + re.escape(source_tag) + r'\b')
    for note_id in affected_note_ids:
        note = session.get(Note, note_id)
        if note and note.content:
            new_content = pattern.sub('#' + target_tag, note.content)
            if new_content != note.content:
                note.content = new_content

    session.commit()

    # Return updated target tag info
    target_min_id_row = session.exec(
        select(func.min(NoteTag.id)).where(
            (NoteTag.user_id == current_user.id) & (NoteTag.tag == target_tag)
        )
    ).one_or_none()
    count = session.exec(
        select(func.count(NoteTag.note_id)).where(
    ...
    return TagManageItem(
        id=target_min_id_row or 0,
        name=target_tag,
        count=count,
        note_ids=note_ids,
    )
...
# ---------- Smart Folders API (CAL-02) ----------


def _apply_smart_folder_filters(query, params: dict, current_user: User, session: Session):
    """Apply stored filter params to a Note query, mirroring /notes endpoint logic."""
    ...
    match_mode = params.get("match", "any")

    parsed_tags = []
    if tag_ids:
        parsed_tags = [t.strip() for t in tag_ids.split(",") if t.strip()]
    has_tag_ids = bool(parsed_tags)

    if has_tag_ids:
        num_tags = len(parsed_tags)
        if match_mode == "all":
            query = (
                query.join(NoteTag, (NoteTag.note_id == Note.id) & (NoteTag.user_id == current_user.id))
                .where(NoteTag.tag.in_(parsed_tags))
                .group_by(Note.id)
                .having(func.count(func.distinct(NoteTag.tag)) >= num_tags)
            )
    ...
    if date_from:
        from_dt = datetime.combine(datetime.fromisoformat(date_from), datetime.min.time())
        query = query.where(Note.created_at >= from_dt)
    if date_to:
        to_dt = datetime.combine(datetime.fromisoformat(date_to), datetime.max.time())
        query = query.where(Note.created_at <= to_dt)
...
@app.get("/smart-folders", response_model=List[SmartFolderRead])
...
def create_smart_folder(
    body: SmartFolderCreate,
    session: Session = Depends(get_session),
    current_user: User = Depends(get_current_user),
):
    """Create a new smart folder (save a named filter preset)."""
    import json

    # Validate filter_params is parseable JSON
    try:
        json.loads(body.filter_params)
    except (json.JSONDecodeError, TypeError):
        raise HTTPException(status_code=422, detail="filter_params must be valid JSON")
...
@app.patch("/smart-folders/{folder_id}", response_model=SmartFolderRead)
def update_smart_folder(
    folder_id: int,
    body: SmartFolderUpdate,
    session: Session = Depends(get_session),
    current_user: User = Depends(get_current_user),
):
    """Update name or filter params of an existing smart folder."""
    folder = session.get(SmartFolder, folder_id)
    if not folder or folder.user_id != current_user.id:
        raise HTTPException(status_code=404, detail="smart folder not found")

    if body.name is not None:
        folder.name = body.name
    if body.filter_params is not None:
        import json
        ...
        try:
            ...
            raise HTTPException(status_code=422, detail="filter_params must be valid JSON")
        folder.filter_params = body.filter_params
...
@app.delete("/smart-folders/{folder_id}", status_code=status.HTTP_204_NO_CONTENT)
...
@app.get("/smart-folders/{folder_id}/notes", response_model=List[NoteRead])
def apply_smart_folder(
    folder_id: int,
    session: Session = Depends(get_session),
    current_user: User = Depends(get_current_user),
):
    """Apply a smart folder's saved filter and return matching notes."""
    import json
    ...
    try:
        params = json.loads(folder.filter_params)
    except (json.JSONDecodeError, TypeError):
        raise HTTPException(status_code=500, detail="smart folder has invalid filter_params")

    query = select(Note).where(Note.user_id == current_user.id)
    query = _apply_smart_folder_filters(query, params, current_user, session)
    notes = session.exec(query).all()
    return [_to_note_read(session, n) for n in notes]
